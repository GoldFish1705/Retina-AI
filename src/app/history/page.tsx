...
import {
  History, TrendingUp, Calendar, AlertCircle,
  CheckCircle2, AlertTriangle, LogIn, Trash2
} from 'lucide-react'
...
interface Scan {
  id: string; grade: number; gradeName: string; riskLevel: string
  confidence: number; description: string; urgency: string; createdAt: string
}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
        <Card className="w-full max-w-md text-center p-8">
          <div className="w-16 h-16 bg-teal-100 rounded-2xl flex items-center justify-center mx-auto mb-4"><History className="w-8 h-8 text-teal-600" /></div>
          ...
          <Button onClick={() => signIn('google')} className="bg-teal-600 hover:bg-teal-700 text-white w-full"><LogIn className="w-4 h-4 mr-2" />เข้าสู่ระบบด้วย Google</Button>
        </Card>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white dark:from-gray-900 dark:to-gray-800">
      ...
      <AnimatePresence>
        {showConfirmClear && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4 text-center">
              ...
              <div className="flex gap-3">
                <Button variant="outline" className="flex-1" onClick={() => setShowConfirmClear(false)}>ยกเลิก</Button>
                <Button className="flex-1 bg-red-500 hover:bg-red-600 text-white" onClick={clearHistory} disabled={clearing}>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">ประวัติการตรวจ</h2>
            <p className="text-sm text-slate-500">{session?.user?.name}</p>
          </div>
          {scans.length > 0 && (
            <Button variant="outline" className="border-red-200 text-red-600 hover:bg-red-50" onClick={() => setShowConfirmClear(true)}>
              <Trash2 className="w-4 h-4 mr-2" />ล้างประวัติ
            </Button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <div className="w-16 h-16 bg-slate-100 dark:bg-gray-700 rounded-2xl flex items-center justify-center mx-auto mb-4"><History className="w-8 h-8 text-slate-400" /></div>
            ...
            <Button onClick={() => router.push('/')} className="bg-teal-600 hover:bg-teal-700 text-white">เริ่มตรวจเลย</Button>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            {chartData.length >= 2 && (
              <Card className="border-slate-200/60 dark:border-gray-700 shadow-sm">
                <CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><TrendingUp className="w-4 h-4 text-teal-600" />แนวโน้มระดับโรค</CardTitle></CardHeader>
                <CardContent>
            ...
            <Card className="border-slate-200/60 dark:border-gray-700 shadow-sm">
              <CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><Calendar className="w-4 h-4 text-teal-600" />ประวัติทั้งหมด ({scans.length} ครั้ง)</CardTitle></CardHeader>
              <CardContent className="p-0">
                <div className="divide-y divide-slate-100 dark:divide-gray-700">
                  {scans.map((scan, i) => {
                    ...
                    return (
                      <motion.div key={scan.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="flex items-center gap-4 p-4 hover:bg-slate-50 dark:hover:bg-gray-700/50 transition-colors">
                        <div className={`w-10 h-10 rounded-xl ${config.bg} flex items-center justify-center flex-shrink-0`}><IconComp className={`w-5 h-5 ${config.color}`} /></div>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-400">{new Date(scan.createdAt).toLocaleDateString('th-TH', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                        </div>
